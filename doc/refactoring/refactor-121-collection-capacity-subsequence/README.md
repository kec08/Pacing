...
- [x] 릴리스 최적화 벤치마크로 checksum과 개선율 기록
...
동일한 100,000개 `TrackPayload` 입력을 15개 표본(표본당 10회)으로 측정하고, 세 번 독립 실행한 중앙값을 기록했다. 측정 중에는 모든 항목의 ID·제목·아티스트를 읽어 결과 checksum이 before/after에서 동일한지 `precondition`으로 검증한다.

| 대상 | Before | After | 개선 |
| --- | ---: | ---: | ---: |
| 배열 capacity 사전 예약 | 1.13684 ms | 0.95025 ms | 16.4% |
| 배치 슬라이스 공유 | 2.75048 ms | 0.21707 ms | 92.1% |
